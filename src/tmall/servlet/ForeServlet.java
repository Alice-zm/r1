...
public class ForeServlet extends BaseServlet {
    ...
    public String register(HttpServletRequest request,HttpServletResponse response){
        ...
        if(userDAO.isExist(name)){
            request.setAttribute("msg","用户名已经使用！");
            return "register.jsp";
        }
        User user=new User();
    ...
    public String buyone(HttpServletRequest request,HttpServletResponse response){
        ...
        for(OrderItem each:orderItems){
           if(product.getId()==each.getProduct().getId()){
               int number=each.getNumber()+num;
               each.setNumber(number);
               orderItemDAO.update(each);
                id=each.getId();
                a=false;
                break;
           }
        ...
        String s = "@user_fore_buy?oiid=" + id;
        return s;
    }
    //购买页面
    public String buy(HttpServletRequest request,HttpServletResponse response){
        String[] ooids=request.getParameterValues("oiid");
        List<OrderItem> items=new ArrayList<>();
        int total=0;
        for(String each:ooids){
             OrderItem orderItem=orderItemDAO.get(Integer.parseInt(each));
             items.add(orderItem);
             total+=orderItem.getNumber()*(orderItem.getProduct().getPromotePrice());
        }
        request.getSession().setAttribute("ois",items);
...
public String changeOrderItem(HttpServletRequest request,HttpServletResponse response){
        ...
        List<OrderItem> ois=orderItemDAO.listByUser(user.getId());
        for(OrderItem each:ois){
            if(each.getProduct().getId()==pid){
                each.setNumber(number);
                orderItemDAO.update(each);
            }
        }
    ...
    public String deleteOrderItem(HttpServletRequest request,HttpServletResponse response){
        int ooid= Integer.parseInt(request.getParameter("oiid"));
        orderItemDAO.delete(ooid);
        return "%success";
    }
    public String createOrder(HttpServletRequest request,HttpServletResponse response){
        ...
        return "@user_fore_alipay?oid="+order.getId() +"&total="+total;
    }
    ...
    public String doreview(HttpServletRequest request, HttpServletResponse response) {
        ...
        return "@user_fore_review?oid="+oid+"&showonly=true";
    }
}
