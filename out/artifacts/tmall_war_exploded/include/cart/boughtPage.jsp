...
$(function(){
	...
	$('#deleteConfirmModal').on('hidden.bs.modal', function (e) {
		if(deleteOrder){
			var page="user_fore_deleteOrder";
			$.post(
				    page,
				    {"oid":deleteOrderid},
				    function(result){
					...
					<tr class="orderItemProductInfoPartTR" >
						...
						<td class="orderItemProductInfoPartTD">
							<div class="orderListItemProductLinkOutDiv">
								<a href="user_fore_product?pid=${oi.product.id}">${oi.product.name}</a>
						...
						<c:if test="${st.count==1}">
							...
							<td valign="top" rowspan="${fn:length(o.orderItems)}" class="orderListItemButtonTD orderItemOrderInfoPartTD" width="100px">
								<c:if test="${o.status=='waitConfirm' }">
									<a href="user_fore_confirmPay?oid=${o.id}">
										<button class="orderListItemConfirm">确认收货</button>
									</a>
								</c:if>
								<c:if test="${o.status=='waitPay' }">
									<a href="user_fore_alipay?oid=${o.id}&total=${o.total}">
...
<%-- 									<button class="btn btn-info btn-sm ask2delivery" link="admin_order_delivery?id=${o.id}">催卖家发货</button> --%>
								...
								<c:if test="${o.status=='waitReview' }">
									<a href="user_fore_review?oid=${o.id}">
